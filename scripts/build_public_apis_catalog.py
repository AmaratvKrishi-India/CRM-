...
REFERENCE_DIR = ROOT / "docs" / "reference" / "public-apis"
SOURCE = REFERENCE_DIR / "public-apis-readme.md"
OUT = REFERENCE_DIR / "Public_APIs_Catalog_and_CRM_Fit.docx"
...
def header_footer(section):
    ...
    add_text(fp, f"Generated from public-apis/public-apis | {date.today().strftime('%d %b %Y')}", 8, False, "666666")
...
def main():
    ...
    no_auth_https = sum(1 for records in data.values() for item in records if item["auth"].strip().lower() == "no" and item["https"] == "Yes")
    generated_label = date.today().strftime("%d %B %Y")
    doc = Document()
    sec = doc.sections[0]
    ...
    add_text(p, f"Catalog snapshot generated: {generated_label} | {total:,} API entries across {len(data)} categories", 10, False, "666666")

    doc.add_heading("What you can use in this project", level=1)
    p = doc.add_paragraph(); p.paragraph_format.space_after = Pt(8)
    add_text(p, f"Filter applied: the catalog contains {no_auth_https:,} entries marked No Auth and HTTPS Yes. That means the repository says they need no account or API key. It does not independently guarantee pricing, uptime, or commercial-use rights, so check each provider's live terms before release. For this CRM, the recommended use cases below are the best direct fits.", 11)
    recs = [
