...
> **Current override — 2026-09-01:** DO NOT SHIP the current working tree. Phase 3 focused sync tests (14/14), TypeScript, local migrations 1–7, and real local RLS checks pass. Mandatory clean install, `npm test`, and production build are blocked by the pre-existing `@accesslint/core@^1.0.0` resolution defect and incomplete local dependencies. The checked historical PASS gates below are evidence for the 2026-08-23 build only, not the current commit/worktree.
